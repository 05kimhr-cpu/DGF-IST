...
Replicates iter 13's design using ALL samples that pass the quality
filter (gold ≥ 8 tokens, non-empty diff ≤ 6000 chars) rather than
capping at PER_LANG = 200.  Available after filtering:
  cpp: 379  cs: 421  go: 339  java: 433  js: 402
  php: 276  py: 420  rust: 342  → total ≈ 3,012

Purpose: tighten Wilson CIs for §5–7 claims and provide a
sample-size sensitivity replication for §8.4.

Artefacts:
  - runs/iter19_scaled_full/generations_{model}.jsonl
  - runs/iter19_scaled_full/results.csv
  - runs/iter19_scaled_full/summary.md
"""
...
OUT_DIR = ROOT / "runs" / "iter19_scaled_full"
...
def summarize(rows: list[dict]) -> str:
    ...
    n_total = len(rows)
    lang_counts = {lang: sum(1 for r in rows if r["language"] == lang) for lang in LANGUAGES}
    lines = [
        "# Iter 19 — Full-sample triad + diff-grounded NLI",
        "",
        f"Samples: {n_total} (all passing filter; iter13 used 200/lang = 1600)",
        "",
        "Per-language counts: " + "  ".join(f"{l}={lang_counts[l]}" for l in LANGUAGES),
        "",
...
def main() -> None:
    t_start = time.time()
    rows = select_samples()
    print(f"selected {len(rows)} samples total", flush=True)
